...
export default function MySeriesScreen() {
  ...
  const ongoingSeries = useMemo(() => allTracking?.filter(t => isOngoing(t.status)) ?? [], [allTracking]);
  const endedSeries   = useMemo(() => allTracking?.filter(t => !isOngoing(t.status)) ?? [], [allTracking]);
          ...
          {ongoingSeries.length > 0 && (
            <View className="mb-6">
              ...
              {ongoingSeries
                ...
                .map(t => <TrackedSeriesCard key={t.seriesId} tracking={t} />)}
            </View>
          )}
